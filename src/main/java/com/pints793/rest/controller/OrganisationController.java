...
public class OrganisationController extends ApplicationSupport {
    ...
    public ResponseEntity<?> getMembers(@RequestHeader("Authorization") String token,
        ...
        Organisation organisation = getOrganisation(organisationId);
        OrganisationMembersResponse response = new OrganisationMembersResponse();

        for (String userId : organisation.getAdminUserIds()) {
            response.addAdmin(userCollection.findById(userId).orElse(null));
        }
        for (String userId : organisation.getMemberUserIds()) {
            if (!response.getAdmins().containsKey(userId)) {
                response.addMember(userCollection.findById(userId).orElse(null));
            }
        }
