...
@Composable
fun MemberProfileScreen(userId: String, onBack: () -> Unit) {
    val vm = remember(userId) { MemberProfileViewModel(userId) }
    val state by vm.state.collectAsStateWithLifecycle()

    PageLayout(title = "Member", onBack = onBack) {
        when {
            ...
            state.profile == null -> Text("Could not load profile.", color = PintsColors.Danger)
                ...
                Row(
                    ...
                    Column {
                        Text(p.name ?: p.username, color = PintsColors.Primary, fontWeight = FontWeight.SemiBold, style = MaterialTheme.typography.titleMedium)
                        Text("@${p.username}", color = PintsColors.TextMuted, style = MaterialTheme.typography.bodySmall)
                        Text(p.email, color = PintsColors.TextMuted, style = MaterialTheme.typography.bodySmall)
                    }
                }
                Spacer(Modifier.height(16.dp))
                ReadField("Display Name", p.name)
